...
/// `TechnicalAttributeCell` is a custom `UICollectionViewCell` designed to display technical attributes in a collection view. It conforms to `FontAdjustable` and `ReusableCellConfiguring` protocols.
final class TechnicalAttributeCell: UICollectionViewCell, FontAdjustable, ReusableCellConfiguring {
    ...
    private func setupLabels() {
        // Setting up nameLabel and scoreLabel with specific frames based on cell size and constants.
        nameLabel = AttributeLabel(
            frame: CGRect(
                x: 0,
                y: 0,
                width: self.frame.width * Constants.nameLabelWidthMultiplier,
        ...
        scoreLabel = AttributeLabel(
            frame: CGRect(
                x: nameLabel.frame.maxX,
                y: 0,
                width: self.frame.width * Constants.scoreLabelWidthMultiplier,
                height: self.frame.height
            )
        )
    ...
    /// Configures the cell with a `TechnicalAttribute` object.
    /// - Parameters:
    ///   - attribute: The `TechnicalAttribute` object used to configure the cell.
    ///   - index: The index of the cell in the collection view.
    func configure(with attribute: TechnicalAttribute, index: Int, option: Int) {
