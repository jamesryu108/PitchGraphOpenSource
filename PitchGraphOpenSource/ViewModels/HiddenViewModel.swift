...
final class HiddenViewModel {
    ...
    private(set) var hiddenAttributesForPlayer1: [HiddenAttribute] = []

    /// Attributes for the second player. Only modifiable within this class and optional.
    private(set) var hiddenAttributesForPlayer2: [HiddenAttribute]?
    ...
    func processData() {
        hiddenAttributesForPlayer1 = player1.getAllHiddenAttributes()
        if let player2 {
            hiddenAttributesForPlayer2 = player2.getAllHiddenAttributes()
        }
    }
}

// MARK: - GoalKeeperConfigurable Conformance
extension HiddenViewModel: HiddenConfigurable {
